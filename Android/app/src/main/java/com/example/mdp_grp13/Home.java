...
//        pathTranslator.translatePath("MOVE,FORWARD,30");
    ...
    // Purely to display a message on the chat box - NOT SEND via BT
...
//            showLog("cmd2 --- " + cmdd[2]);
            ...
            if(message.contains("ROBOT")) {
                ...
                String[] sentCoords = Arrays.copyOfRange(cmd, 1, 3);
                String sentDirection = cmd[3];
//                BluetoothCommunications.getMessageReceivedTextView().append("\n");
                String direction = "";
                if (sentDirection.contains("EAST")) {
                    direction = "right";
                }
                else if (sentDirection.contains("NORTH")) {
                    direction = "up";
                }
                else if (sentDirection.contains("WEST")) {
                    direction = "left";
                }
                else if (sentDirection.contains("SOUTH")) {
                    direction = "down";
                }
                else{
                    direction = "";
            ...
            //image format from RPI is "TARGET,<obID>,<ImValue>" eg TARGET,3,7
                ...
                try {
                    ...
                    if (Integer.valueOf(cmd[2].replace("\n","")) >= 11 && Integer.valueOf(cmd[2].replace("\n","")) <=40){
...
//                updateStatus(cmd[0]+" "+ cmd[1]+" "+cmd[2]);
            ...
            //Expects a syntax of eg. MOVE,<DIRECTION>,<DISTANCE IN BLOCKS>.
            //Expects a syntax of eg. TURN,<DIRECTION>.
