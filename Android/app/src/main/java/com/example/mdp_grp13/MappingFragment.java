...
public class MappingFragment extends Fragment {
    ...
    Button updateButton, buttonSendObstacles;
...
//        updateButton = root.findViewById(R.id.updateMapBtn);
        buttonSendObstacles = root.findViewById(R.id.sendObstaclesButton);
...
//                BluetoothCommunications.getMessageReceivedTextView().append(direction);
        ...
        // To send all obstacle locations to rpi
        buttonSendObstacles.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                showLog("Clicked buttonSendObstacles");

                // Convert each arraylist<int[]> to a string
                StringBuilder coordString = new StringBuilder();
                // Iterate through each int[] in the ArrayList
                ArrayList<int[]> obstaclesList = gridMap.getObstaclesList();
                for (int i = 1; i <= obstaclesList.size(); i++) {
                    int[] coord = obstaclesList.get(i-1);

                    // Append obstacle information, including the item number (i)
                    coordString.append("OBSTACLE," + coord[0] + "," + coord[1] + ",");
                    coordString.append(gridMap.getImageBearings(coord[0], coord[1])).append("," + i + "|");
                }

                // Print the string using the printMessage function
                coordString.deleteCharAt(coordString.length() - 1);
                Home.printMessage(coordString.toString().trim());

                // Show a log message after finishing
                showLog("Exiting buttonSendObstacles");
